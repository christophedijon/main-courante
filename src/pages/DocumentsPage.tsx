...
const CATEGORIES: { id: Categorie; label: string; icon: React.ComponentType<{ className?: string }>; accent: string; activeClass: string; cardClass: string }[] = [
  {
    id: 'RONDE',
    label: 'Ronde',
    icon: Shield,
  ...
  },
  {
    ...
    icon: Flame,
    accent: 'text-red-400',
    activeClass: 'border-red-500 bg-red-500/10 text-red-300',
  ...
  },
  {
    ...
    activeClass: 'border-slate-400 bg-slate-700/50 text-slate-200',
    cardClass: 'border-slate-600/30 bg-slate-800/30',
  },
  {
    id: 'RADIO',
    label: 'Radio',
...
export default function DocumentsPage() {
  ...
  async function handleSave(e: FormEvent) {
    ...
    if (editTarget) {
      ...
      ({ error } = await supabase.from('toolbox_documents').insert(payload));
    }
    setSaving(false);
  ...
  async function moveOrdre(doc: Doc, dir: 'up' | 'down') {
    ...
    const newOrdreDoc = other.ordre;
    const newOrdreOther = doc.ordre;
    await Promise.all([
      supabase.from('toolbox_documents').update({ ordre: newOrdreDoc, updated_at: new Date().toISOString() }).eq('id', doc.id),
      supabase.from('toolbox_documents').update({ ordre: newOrdreOther, updated_at: new Date().toISOString() }).eq('id', other.id),
    ]);
    setDocs((prev) => prev.map((d) => {
      if (d.id === doc.id) return { ...d, ordre: newOrdreDoc };
      if (d.id === other.id) return { ...d, ordre: newOrdreOther };
      return d;
    }));
  }
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Contenus publiés dans la Boîte à outils mobile
            </p>
          </div>
          <button
            onClick={openCreate}
            ...
            return (
              ...
              >
                ...
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold
                  ${active ? 'bg-white/15' : 'bg-slate-700 text-slate-400'}`}>
                  {count}
                </span>
              </button>
            );
                  ...
                  <div key={doc.id} className={`${!doc.actif ? 'opacity-50' : ''}`}>
                    <div className="px-5 py-4 flex items-start gap-4 group">
                      ...
                      <div className="flex flex-col gap-0.5 mt-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button
                          type="button"
                          disabled={idx === tabDocs.length - 1}
                          onClick={() => moveOrdre(doc, 'down')}
                          className="w-6 h-6 rounded-lg text-slate-500 hover:text-white hover:bg-slate-700 flex items-center justify-center disabled:opacity-20 disabled:cursor-default transition-all"
                        >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <span className="text-xs text-slate-600">#{doc.ordre}</span>
                        </div>
                        {doc.description && (
                      ...
                      <div className="flex items-center gap-1 shrink-0 ml-2 opacity-0 group-hover:opacity-100 transition-all">
                        <button
                          ...
                          title={doc.actif ? 'Désactiver' : 'Activer'}
