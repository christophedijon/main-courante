import { useEffect, useState } from 'react';
import { useNavigate, useParams, Navigate } from 'react-router-dom';
import { supabase } from '../../lib/supabase';
import { useSaisie, SaisieType } from './SaisieContext';
import StepHeader from '../components/StepHeader';
import SelectableTile from '../components/SelectableTile';

export default function StepEspace() {
  const { type } = useParams<{ type: SaisieType }>();
  const navigate = useNavigate();
  const { draft, setField } = useSaisie();
  const [items, setItems] = useState<{ id: string; nom: string; description: string | null }[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    supabase.from('espaces').select('id, nom, description').order('nom').then(({ data }) => {
      setItems(data ?? []);
      setLoading(false);
    });
  }, []);

  if (!type) return <Navigate to="/mobile" replace />;
  if (!draft.etablissement) return <Navigate to={`/mobile/saisie/${type}/etablissement`} replace />;

  function pickItem(id: string, nom: string) {
    setField('espace', { id, label: nom });
    setField('zone', null);
    navigate(`/mobile/saisie/${type}/zone`);
  }

  return (
    <div>
      <StepHeader
        step={2} total={7}
        title="Choisir l'espace"
        subtitle={draft.etablissement.label}
        backTo={`/mobile/saisie/${type}/etablissement`}
      />
      <div className="px-4 py-5 space-y-2.5">
        {loading && <p className="text-slate-500 text-sm text-center py-8">Chargement…</p>}
        {!loading && items.length === 0 && (
          <p className="text-slate-500 text-sm text-center py-8">Aucun espace configuré.</p>
        )}
        {items.map((e) => (
          <SelectableTile
            key={e.id}
            label={e.nom}
            description={e.description || undefined}
            selected={draft.espace?.id === e.id}
            onClick={() => pickItem(e.id, e.nom)}
          />
        ))}
      </div>
    </div>
  );
}
